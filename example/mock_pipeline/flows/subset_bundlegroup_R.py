...
TaskListChoices = list[Literal["combo50_list","do_nothing_bundle_group_R",]]|list[Never]

def subset_bundlegroup_R(pipeline_config: PipelineConfig, task_list: TaskListChoices = [], ignore_cache: bool = False):
    ...
    _do_nothing_bundle_group_R = submit(
...
if __name__ == "__main__":
    ...
    pipeline_config = PipelineConfig(
        ...
        PIPELINE_NAME="subset_bundlegroup_R",
        PY_MODULE_PATH=tasks.__name__,
        R_TASKS_DIRS=(str(Path(__file__).parent / "../r_tasks"),),
    )
    subset_bundlegroup_R(pipeline_config, ignore_cache=args.force)
