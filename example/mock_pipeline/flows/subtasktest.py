...
TaskListChoices = list[Literal["subtask_list","subtask_process",]]|list[Never]

def subtasktest(pipeline_config: PipelineConfig, task_list: TaskListChoices = [], ignore_cache: bool = False):
    ...
    _subtask_process = submit(
        "subtask_process",
        pipeline_config,
        task_list,
        ignore_cache,
        tasks.subtask_process
    )
...
if __name__ == "__main__":
    ...
    pipeline_config = PipelineConfig(
        ...
        PIPELINE_NAME="subtasktest",
        PY_MODULE_PATH=tasks.__name__,
        R_TASKS_DIRS=(str(Path(__file__).parent / "../r_tasks"),),
    )
    subtasktest(pipeline_config, ignore_cache=args.force)
