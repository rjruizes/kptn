...
TaskListChoices = list[Literal["combo50_list","combo_process_bundle_group_R",]]|list[Never]

def bundlegroup_R(pipeline_config: PipelineConfig, task_list: TaskListChoices = [], ignore_cache: bool = False):
    ...
    _combo_process_bundle_group_R = submit(
...
if __name__ == "__main__":
    ...
    pipeline_config = PipelineConfig(
        ...
        PIPELINE_NAME="bundlegroup_R",
        PY_MODULE_PATH=tasks.__name__,
        R_TASKS_DIRS=(str(Path(__file__).parent / "../r_tasks"),),
    )
    bundlegroup_R(pipeline_config, ignore_cache=args.force)
