...
TaskListChoices = list[Literal["combo50_list","combo_process_bundle_group",]]|list[Never]

def bundlegroup(pipeline_config: PipelineConfig, task_list: TaskListChoices = [], ignore_cache: bool = False):
    ...
    _combo_process_bundle_group = submit(
        "combo_process_bundle_group",
        pipeline_config,
        task_list,
        ignore_cache,
        tasks.combo_process_bundle_group
    )
...
if __name__ == "__main__":
    ...
    pipeline_config = PipelineConfig(
        ...
        PIPELINE_NAME="bundlegroup",
        PY_MODULE_PATH=tasks.__name__,
        R_TASKS_DIRS=(str(Path(__file__).parent / "../r_tasks"),),
    )
    bundlegroup(pipeline_config, ignore_cache=args.force)
