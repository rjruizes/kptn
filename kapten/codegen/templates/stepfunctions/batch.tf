...
resource "aws_batch_compute_environment" "kapten" {
...
resource "aws_batch_job_queue" "kapten" {
  for_each = var.create_batch_resources ? { main = true } : {}
  ...
  compute_environment_order {
    order               = 1
    compute_environment = aws_batch_compute_environment.kapten["main"].arn
  }
...
resource "aws_batch_job_definition" "kapten" {
  for_each = var.create_batch_resources ? { main = true } : {}
