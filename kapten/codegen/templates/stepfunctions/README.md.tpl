...
- `lambda.tf` provisions the Kapten decider Lambda (or wires an existing one when provided).
...
3. Run `kapten bundle-decider --output-dir infra/lambda_decider` whenever Kapten or project code changes. The command installs your local Kapten checkout by default; use `--no-prefer-local-kapten` to pull from PyPI and `--install-project` to add your project package. The scaffolding command runs it once automatically.
