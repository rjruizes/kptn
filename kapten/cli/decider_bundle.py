...
from kapten.util.pipeline_config import _module_path_from_dir, normalise_dir_setting
...
def _resolve_python_dirs(project_root: Path, kap_conf: Mapping[str, object]) -> list[Path]:
    ...
    if not isinstance(settings, Mapping):
        raise BundleDeciderError("kapten.yaml settings block must be a mapping")
...
def _resolve_local_kapten_root() -> Path | None:
    candidate = Path(__file__).resolve().parents[2]
    if (candidate / "kapten" / "aws" / "decider.py").exists():
        return candidate
    return None
...
def bundle_decider_lambda(
    *,
    ...
    kapten_source: str | Path | None = None,
    ...
    prefer_local_kapten: bool = True,
) -> BundleResult:
    """Build the Kapten decider Lambda bundle for the given project."""
    project_root = project_root.resolve()
    kapten_config_path = project_root / "kapten.yaml"
    if not kapten_config_path.exists():
        raise BundleDeciderError(f"kapten.yaml not found in {project_root}")

    kap_conf = read_tasks_config(str(kapten_config_path))
    ...
    chosen_kapten_source = kapten_source
    if prefer_local_kapten and not chosen_kapten_source:
        candidate = _resolve_local_kapten_root()
        if candidate is not None:
            chosen_kapten_source = candidate

    if chosen_kapten_source:
        install_targets.append(str(chosen_kapten_source))
    else:
        install_targets.append("kapten")
    ...
    for directory in pruned_dirs:
        try:
            ...
            raise BundleDeciderError(
                f"Directory {directory} is outside the project root; adjust kapten.yaml to use project-relative paths"
            ) from exc
        dest = bundle_dir / rel
    ...
    for file_path in sorted(task_files):
        ...
        try:
            ...
            raise BundleDeciderError(
                f"File {file_path} is outside the project root; adjust kapten.yaml to use project-relative paths"
            ) from exc
        dest = bundle_dir / rel
        dest.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(file_path, dest)

    shutil.copy2(kapten_config_path, bundle_dir / "kapten.yaml")

    handler_path = bundle_dir / "lambda_function.py"
    handler_path.write_text(
        "from kapten.aws.decider import handler as _handler\n\n\n"
