...
def validate_kapten_config(config_path: Path, schema_path: Path) -> List[ValidationIssue]:
    """
    Validate a kapten configuration file against the schema.

    Returns a list of issues; the list is empty when validation succeeds.
    """
    if not config_path.exists():
        raise FileNotFoundError(f"kapten.yaml not found at {config_path.resolve()}")

    if not schema_path.exists():
        raise FileNotFoundError(f"kapten-schema.json not found at {schema_path.resolve()}")
    ...
    try:
        ...
        raise SchemaValidationError(f"Schema error in kapten-schema.json: {exc}") from exc
