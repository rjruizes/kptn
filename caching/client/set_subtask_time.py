...
def set_time_in_subitem_in_databin(
    ...
    """
    Update the databin item at the given index for the given field in the DynamoDB table using boto3.client.
    ...
    key = {
        ...
        "SK": {"S": f"PIPELINE#{pipeline_id}#TASK#{task_id}#SUBTASKBIN#{databin}"},
    }
    ...
    try:
        ...
        print(f"Error updating subtask in databin: {e.response['Error']['Message']}")
        raise
